...
#checking if keypoints has 'NaN' values
...
# this function plots an image with corresponds to its keypoints

def visualize_points(img, points):
    fig, ax = plt.subplots(1)
    ax.set_aspect('equal')  # Corrected method name
    ax.imshow(img)
    for i in range(0, len(points), 2):
        x_renorm = (points[i] + 0.5) * 96  # Denormalize x-coordinate
        y_renorm = (points[i + 1] + 0.5) * 96  # Denormalize y-coordinate
        circ = plt.Circle((x_renorm, y_renorm), 1, color='r')  # Plot the keypoints at the x and y coordinates
        ax.add_patch(circ)
    plt.show()
...
test = pd.read_csv('data/test (8)/test.csv')
